...
describe("pageInfo - forward pagination", () => {
  it("reports hasNextPage/hasPreviousPage from the window position", async () => {
    ...
    const page = async (args: string) => {
      ...
      validateResult(r);
      return r.data.models.Child;
    };

    const p1 = await page("first: 2");
    expect(p1.edges.map((e: any) => e.node.name)).toEqual(["c1", "c2"]);
    ...
    expect(afterFirst.edges.map((e: any) => e.node.name)).toEqual(["c2", "c3"]);
    ...
    expect(lastPage.edges.map((e: any) => e.node.name)).toEqual(["c5"]);
