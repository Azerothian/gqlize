  ...
  return async function resolve(source: AdapterRow, args: MutationModelArgs, context: RequestContext, info: GraphQLResolveInfo) {
    ...
    if (args.update) {
      results = await waterfall(args.update, async(arg, arr) => {
        ...
        const node = await waterfall(result, (el) => processAfter(el, args, context, info, definition, Events.MUTATION_UPDATE));
        return arr.concat(node);
      }, results);
    }
    if (args.delete) {
      results = await waterfall(args.delete, async(arg, arr) => {
        ...
        const node = await waterfall(result, (el) => processAfter(el, args, context, info, definition, Events.MUTATION_DELETE));
        return arr.concat(node);
      }, results);
    }
    if (args.select) {
