 ...
 * fail closed — it silently generates schemas looser than their author asked
 * for. `Permission` is therefore a closed type: the excess-property check on an
 * object literal is what turns the typo into a compile error. If `Permission`
 * ever regains an index signature the `@ts-expect-error` below stops firing and
 * `tsc -p tsconfig.test.json` fails with TS2578.
 */
describe("ormize-zod4 - permission typing", () => {
