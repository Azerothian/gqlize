...
| [`@azerothian/temporalize`](packages/temporalize) | Temporal projection — `createActivities`/`createWorkers` turn each model's CRUD surface, `classMethods` and `instanceMethods` into Temporal activities on a per-model task queue, with a required per-call context, an optional permission gate and generic CRUD workflows. |
...
Dependency graph (acyclic): `graphql-types` + `utilize` → `ormize` → { `gqlize`, `ormize-zod4`, `nestize`, `temporalize` } ; the sequelize adapter implements `GqlizeAdapter` (from `@azerothian/gqlize`) and is registered on an `Ormize`. `gqlize` (GraphQL), `ormize-zod4` (Zod), `nestize` (REST) and `temporalize` (Temporal) are four projections of one `Ormize` instance.
...
| [`examples/valkey-basic`](examples/valkey-basic) | A Valkey/Redis-backed ormize instance: an index-only query, a relationship read via the foreign-key index map, and object expiry cascading into the index maps (ephemeral in-process redis, no server needed). | `pnpm --filter @azerothian/example-valkey-basic start` |
...
examples/
    ...
    ├── temporalize-worker/         # Temporal workers + client example
